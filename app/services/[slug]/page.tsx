...
const pages: ServicePage[] = [
  {
    ...
    sections: [
      ...
      { heading: "What to expect", body: "Bring a photo ID, your insurance card if you have one, and a list of current medications. The care team will evaluate you, explain recommended next steps, and advise when follow-up or emergency care is appropriate." },
    ],
  },
  {
    slug: "x-ray-lab",
  ...
  },
  {
    ...
    sections: [
      ...
      { heading: "Clear next steps", body: "The provider will complete the evaluation and explain any follow-up or additional documentation needed for your situation." },
    ],
  },
  {
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return page ? { title: page.title, description: page.description, alternates: { canonical: `/services/${page.slug}` } } : {};
}
    ...
    <article className="bg-ivory">
      <header className="bg-navy-950 px-5 pb-16 pt-36 text-ivory lg:px-8">
        <div className="mx-auto max-w-4xl">
          <p className="text-xs font-bold uppercase tracking-[0.3em] text-sky">Urgent Care of Ennis</p>
          <h1 className="font-display mt-4 max-w-3xl text-4xl font-black tracking-tight sm:text-5xl">{page.title}</h1>
          <p className="mt-6 max-w-2xl text-lg leading-relaxed text-ivory/80">{page.intro}</p>
          <div className="mt-8 flex flex-wrap gap-3">
            <a href={site.phoneHref} className="rounded-xl bg-red px-6 py-3 text-sm font-bold text-ivory">Call Now: {site.phone}</a>
            <a href={site.address.mapsUrl} target="_blank" rel="noopener noreferrer" className="rounded-xl border border-ivory/30 px-6 py-3 text-sm font-bold text-ivory">Get Directions</a>
          </div>
        </div>
      </header>
      <div className="mx-auto grid max-w-4xl gap-10 px-5 py-16 lg:grid-cols-[1fr_260px] lg:px-8">
        <div className="space-y-10">
          {page.sections.map((section) => <section key={section.heading}><h2 className="font-display text-2xl font-black text-navy-900">{section.heading}</h2><p className="mt-3 leading-relaxed text-ink-soft">{section.body}</p></section>)}
          <p className="rounded-xl border border-red/20 bg-red/5 p-4 text-sm leading-relaxed text-ink-soft">This information is general and does not replace medical advice. For a life-threatening emergency, call 911 or go to the nearest emergency room.</p>
        </div>
        <aside className="h-fit rounded-2xl border-2 border-navy-900/10 bg-white p-6">
          <h2 className="font-display text-lg font-bold text-navy-900">Visit the clinic</h2>
          <p className="mt-3 text-sm leading-relaxed text-ink-soft">{site.address.line1}<br />{site.address.line2}</p>
          <p className="mt-3 text-sm font-semibold text-navy-900">{site.hours}</p>
          <nav className="mt-6 grid gap-2 text-sm font-bold" aria-label="Related pages"><Link href="/services" className="text-red hover:underline">All services</Link><Link href="/contact" className="text-red hover:underline">Contact and directions</Link><Link href="/#insurance" className="text-red hover:underline">Insurance and pricing</Link></nav>
        </aside>
      </div>
    </article>
  );
